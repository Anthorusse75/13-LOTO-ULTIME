...
- [x] Composants custom Tailwind (remplacement Shadcn/ui)
...
- [x] CooccurrenceTab (affiche paires top/bottom affinité)
...
- [x] Navigation clavier (focus-visible, skip-link, aria-labels)
...
- [x] Dockerfile frontend (Nginx + curl healthcheck)
- [x] docker-compose.yml (API + frontend + observabilité)
- [x] .dockerignore (backend + frontend)
- [x] README.md complet (installation, config, usage, API, Docker)
- [x] CONTRIBUTING.md (workflow Git, conventions commits, tests)
- [x] Performance audit (event loop fix, NullPool SQLite, asyncio.to_thread)
- [x] Optimisations identifiées appliquées (scoring async, pool exhaustion fix)
- [x] Couverture tests globale ≥ 80% (84.65% backend, 455 tests)
- [x] Swagger/OpenAPI à jour (/api/v1/docs + /api/v1/redoc)
- [x] Seed data pour démo (5 jeux YAML + admin auto + pipeline initial)
- [x] Revue sécurité OWASP Top 10 (JWT, RBAC, rate limiting, headers, ORM)
- [x] Données sensibles dans .env uniquement (12-factor app)
- [x] .env.example à jour (toutes les variables documentées)
- [x] Tag git v1.0.0
- [x] Changelog v1.0.0
...
| 7. Interface Frontend           | 39      | ✅ Complète    |
| 8. Scheduler & Jobs             | 16      | ✅ Complète    |
| 9. Sécurité & Auth              | 19      | ✅ Complète    |
| 10. Polish & Déploiement        | 16      | ✅ Complète    |
| **TOTAL**                       | **199** | **✅ 100%**   |
