...
authTest.describe("Grids generation and consultation", () => {
  ...
  authTest(
    "generates grids and shows results",
    async ({ loggedInPage: page }) => {
      ...
      await expect(page.getByText(/grilles générée|5 grilles/i)).toBeVisible({
        timeout: 60_000,
      });
    },
  );
  ...
  authTest(
    "can select a grid to see detail panel",
    async ({ loggedInPage: page }) => {
      await page.goto("/grids");
      ...
      await page.waitForSelector(".rounded-md.cursor-pointer", {
        timeout: 60_000,
      });
  ...
  authTest(
    "favorites page loads without error",
    async ({ loggedInPage: page }) => {
