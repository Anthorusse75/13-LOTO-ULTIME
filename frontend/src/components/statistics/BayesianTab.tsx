...
export default function BayesianTab() {
  ...
  if (!bayesian || bayesian.length === 0)
    return (
      <p className="text-text-secondary">
        Aucune donnée bayésienne disponible.
      </p>
    );

  const sorted = [...bayesian].sort(
  ...
  const chartData = sorted.slice(0, 20).map((b) => ({
    ...
    errorRange: [
      b.posterior_mean - b.ci_95_low,
      b.ci_95_high - b.posterior_mean,
    ],
  }));
    ...
    <div className="space-y-6">
      ...
      <div className="bg-surface rounded-lg border border-border p-4">
        ...
        <p className="text-xs text-text-secondary mt-2">
          Moyenne postérieure (modèle Beta-Binomial) avec intervalle de
          confiance à 95%.
        </p>
      </div>
