      ...
      {stats?.star_frequencies && stats.star_frequencies.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-lg font-semibold flex items-center gap-2">
            <span className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-accent-purple/20 text-accent-purple text-xs">★</span>
            Fréquences {game?.star_name ?? "Étoiles / N° Chance"}
          </h2>
          <div className="bg-surface rounded-lg border border-accent-purple/30 p-4">
            <ResponsiveContainer width="100%" height={250}>
              <BarChart data={[...stats.star_frequencies].sort((a, b) => a.number - b.number)}>
                <XAxis
                  dataKey="number"
                  tick={{ fill: "var(--color-text-secondary)", fontSize: 12 }}
                />
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-surface-hover">
                  ...
                  <th className="px-4 py-2 text-right text-text-secondary">Tirages</th>
                  <th className="px-4 py-2 text-right text-text-secondary">Fréquence</th>
                  <th className="px-4 py-2 text-right text-text-secondary">Ratio</th>
                  <th className="px-4 py-2 text-right text-text-secondary">Dernier vu</th>
                </tr>
              </thead>
              <tbody>
                {[...stats.star_frequencies].sort((a, b) => b.count - a.count).map((f) => (
                  <tr key={f.number} className="border-b border-border hover:bg-surface-hover">
                    ...
                    <td className="px-4 py-2 text-right font-mono">{f.count}</td>
                    <td className="px-4 py-2 text-right font-mono">{f.relative.toFixed(4)}</td>
                    <td className="px-4 py-2 text-right font-mono">{f.ratio.toFixed(3)}</td>
                    <td className="px-4 py-2 text-right font-mono text-text-secondary">
                      il y a {f.last_seen} tirages
                    </td>
                  </tr>
