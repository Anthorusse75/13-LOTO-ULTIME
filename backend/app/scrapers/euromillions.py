"""EuroMillions scraper — fetches draw results from the FDJ historical CSV ZIP."""
...
# Latest archive: Feb 2020 — present
EUROMILLIONS_ZIP_URL = (
...
class EuroMillionsScraper(BaseScraper):
    """Scraper for EuroMillions draw results from official CSV archives."""

    def __init__(self, zip_url: str = EUROMILLIONS_ZIP_URL, timeout: float = 60.0):
        self._zip_url = zip_url
        self._timeout = timeout

    async def fetch_latest_draws(self, since_date: date | None = None) -> list[RawDraw]:
        """Fetch EuroMillions draws since given date (default: all available)."""
