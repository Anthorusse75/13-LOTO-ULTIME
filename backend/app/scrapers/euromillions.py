"""EuroMillions scraper — fetches draw results from the FDJ/EuroMillions API."""
...
EUROMILLIONS_API = "https://www.fdj.fr/api/euromillions/results"


class EuroMillionsScraper(BaseScraper):
    """Scraper for EuroMillions draw results."""

    def __init__(self, base_url: str = EUROMILLIONS_API, timeout: float = 30.0):
        self._base_url = base_url
        self._timeout = timeout

    async def fetch_latest_draws(self, since_date: date | None = None) -> list[RawDraw]:
        """Fetch EuroMillions draws since given date (default: last 30 days)."""
        if since_date is None:
            since_date = date.today() - timedelta(days=30)

        log = logger.bind(scraper="euromillions", since=str(since_date))
    ...
    def _parse_entry(self, entry: dict) -> RawDraw:
        ...
        stars = entry.get("stars", entry.get("etoiles", []))
        ...
        return RawDraw(
            ...
            stars=sorted([int(s) for s in stars]) if stars else None,
        )
